...
class Machine
    ...
    def Purchase_Check?(suica,name)
      suica.balance >= @Stock_Juce[name][:price] && @Stock_Juce[name][:quantity] > 0
    end
    ...
    def Purchase(suica,name)
      if Purchase_Check?(suica,name)
        ...
        @Stock_Juce[name][:quantity]
        #売り上げ金額を増やす
        @sales += @Stock_Juce[name][:price]

        #Suicaのチャージ金額を減らす
        suica.pay(@Stock_Juce[name][:price])
    ...
    # 自動販売機は購入可能なドリンクのリストを取得できる
     def buy_drink_list(suica)
      ...
      @Stock_Juce.each do |key,value|
        if suica.balance >= value[:price] && value[:quantity] > 0
          list << key
        end
